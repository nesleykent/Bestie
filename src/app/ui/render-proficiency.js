...
export function renderProficiency(container, session, { processed, sort, plans, activeId, projectionCreature, expanded = false }) {
    ...
    container.innerHTML = `
        ...
        <section class="results-section" aria-labelledby="proficiencyBreakdownTitle">
            ...
            ${rows.length ? `<div class="table-container proficiency-table" tabindex="0" role="region" aria-label="Creature and boss proficiency breakdown, scroll horizontally for all columns">
                ...
                ${rows.length > 5 ? `<button class="text-action" id="proficiencyExpand" type="button" aria-expanded="${expanded}">${expanded ? "Show fewer creatures" : `Show all ${rows.length} creatures`}</button>` : ""}
