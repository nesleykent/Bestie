...
Last updated: 2026-09-18. Baseline: `a2359f7`. Status: **IN PROGRESS — not PROJECT_COMPLETE**.
...
| C02 DEFECT / TEST GAP | Safe persisted state and backup/restore; README ownership, UX recovery | Strict preflight validates nested progress, tool drafts, plans and undo; unsupported fields/versions reject before replacement. Legacy numeric undo normalizes. Damaged browser records remain untouched with saving paused. | Shared validator on imports and stored workspaces; strict vendor booleans; original recovery downloads | 15 backup boundary tests, legacy/full roundtrip, Chrome restore/recovery and nested corrupt-record retention passed | VERIFIED |
...
| C05 DEFECT / TEST GAP | Honest Bestiary/Task estimates and optimal Charm Plan; README formulas | Planning now preserves unknown, reviewed zero and stage bounds; opportunity rates are respawn-separated; all-session rewards deduplicate; exact session input overrides tiles and supports undo | Shared evidence shape, conservative projections and explicit uncertainty across session/plan/opportunity UI | Independent 120-case optimizer oracle plus 11 evidence tests; Chrome unknown/zero/malformed reload, all-session view, mode selection, desktop/mobile passed | VERIFIED |
| C06 CONFIRMED REQUIREMENT | Charmwise; README Analysis expansion | Tools → Charmwise reads canonical stages and separate currencies, optional balances/promotion and target affinity with explicit uncertainty | Shared tracker rules; no inferred stats, spending or assignment; contextual explanations | Four dataset-backed tests, isolated Chrome at 1440px/390px, reload and invalid balances; screenshots inspected | VERIFIED |
| C07 CONFIRMED REQUIREMENT / DATA GAP | Curated Hunt Grounds and objective-based Hunt Planner; README Data/Analysis expansion | Tools → Hunt Planner has 880 source entries, level/vocation/party/objective filters, paginated comparison and explicit saved-session links | Deterministic importer from pinned MIT TibiaPal current tables, source manifest/notice; local measured rates duration-weighted and respawn-separated | All 880 records validated; independent weighted-rate tests; Chrome filters, link, source separation, reload, invalid level and desktop/mobile passed | VERIFIED |
| C08 CONFIRMED REQUIREMENT | Repository character configuration and automated Character History; README data model | Config, validated source adapters, scheduled pipeline and Data → Character History implemented; public snapshots separate from private roster | All-vocation public highscore pagination, source-age server-save dates, profile/death/XP/rank history and honest unavailable states | Fixture orchestration/failure/identity tests; live TibiaData profile/highscore run; Chrome empty/populated/stale/escaping/responsive views | VERIFIED |
| C09 CONFIRMED REQUIREMENT | World-specific Market Watch; README data expansion | World-scoped selected-item pipeline, source-time freshness and Data → Market Watch implemented; explicit fresh quote fill in Imbuements | Optional repository token, no browser credentials; reconcile identity even when token absent | Mock authenticated API boundaries, partial failures, item removal/World change, stale/future rejection; Chrome manual price preservation/reload. Live authenticated refresh needs deployment configuration | VERIFIED |
...
| C11 CONFIRMED REQUIREMENT | Imbuements tool; README Planned tools | 24 types, three cumulative tiers, current guaranteed fees, token/hybrid comparison, manual prices and optional fresh Market quotes | Independent calculation engine, attributed factual recipes, explicit unavailable prices and personal benefit estimates | Independent cost/fee/token/missing/invalid tests; Chrome tier costs, Punch, reload, quotes and mobile/desktop | VERIFIED |
| C12 CONFIRMED REQUIREMENT | Minimap Markers local merge/conversion/backups; README Planned tools | Tools → Minimap Markers implements file preview, personal-priority merge, conversion, original-byte backup and report | Independent engine, community icon compatibility, explicit supported format limits; original file never overwritten | 44 tests including primary TibiaMaps hexdump; desktop/mobile Chrome confirms personal priority, backup/merged download bytes, escaping and invalid input blocks export | VERIFIED |
| C13 CONFIRMED REQUIREMENT | Morning Tibia World Board / Guide parsing; README Planned tools | Tools → Morning Tibia processes World Board/Guide text with world/date, preserved processed evidence, explicit manual corrections and tentative wording | Source-exact 14/26 catalog projected from MIT reference with separate transcript attribution; five upstream unverified flags retained | Full/partial/malformed/variant/latest/tentative/chatter tests; browser validation of corrections, draft isolation, reload and mobile | VERIFIED |
...
| C15 CONFIRMED REQUIREMENT / PARTIAL IMPLEMENTATION | Data Management selected/all clearing, migration, export/restore; README expansion | Data Management now provides active-character scoped clears, counts, confirmation, recovery downloads and tracker undo | Clears sessions/associations, tracker records, plan inputs or tools independently; other characters and repository snapshots remain separate | Three scope/undo tests; Chrome cancel, confirmed clear, recovery download/restore, desktop/mobile and no runtime errors passed | VERIFIED |
...
| C20 DATA GAP | Snapshot completeness, provenance and updateability; README bundled datasets | Seven snapshots audited; identity/threshold/cost tests, case-insensitive location join preserves keys; source capture dates and older rarity dates distinguished | 93 named questlogs/104 ungrouped documented; source manifest/update contracts; unavailable boss metadata remains unknown; further derived achievements optional | 22 dataset integrity tests including nonzero foreign progress isolation; 147 location joins; rarity date shown in details | IMPLEMENTING |
...
- Local file/world tools: full suite 188/188, lint/build passed. Chrome verified original marker backup bytes, merged binary decode, personal-priority conflict preview, invalid-source export blocking, literal text rendering, and 390px/1440px layouts. Morning Tibia verified confirmed/tentative/unknown states, manual correction, changed draft World and invalid draft preserving prior evidence, reload and mobile. Corrected bracketed timestamps and mobile textarea after inspection. Primary replaced hand-transcribed world data with an exact projection of the inspected upstream definitions/messages.

- Hunt Planner delivery: full suite 193/193, lint/build pass. Reproducible 880-row MIT TibiaPal projection replaces the unfinished hand-selected draft. Isolated Chrome verified reference 60,000 raw XP/h versus independently linked local 1,000 raw XP/h, filter persistence, malformed level rejection, and desktop/mobile layouts. Source manifest added; remaining original snapshot audit remains C20.

- Resume delivery: Charmwise, bounded planning evidence and Data Management verified in isolated Chrome. Original HTTP test server reset parallel module requests; a 128-connection server queue removed that test-environment failure. Session exact-zero and malformed-input persistence covered; shared tool checkboxes corrected after screenshot inspection. Screenshots under `/private/tmp/{charmwise,planning,manage}-*.png`.

- Integrity continuation (2026-09-18): unsupported tracker fields now fail before destructive normalization; stage ceilings follow each tracker. Legacy undo counts normalize before use. Full suite 337/337 including concurrent data audit; scoped backup/public/imbuement suite 85/85. Chrome recovery roundtrip and nested corrupt original retention passed.

- Public data and Imbuements delivery: isolated Chrome passed blank/configured history, XP delta, source text escaping, stale market quotes, 24 recipes, manual prices, token arithmetic, fresh quote fill preserving manual values, persistence and responsive layout. Mobile table labels corrected after screenshot inspection. Live TibiaData profile/highscore pipeline succeeded against a disposable config; no real character was configured or private data published. Market end-to-end requests remain fixture-verified because no deployment token is configured.
